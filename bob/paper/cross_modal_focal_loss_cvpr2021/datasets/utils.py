...
class RandomChannelErase(object):

  """Erase a channel from a multi channel image randomly. atmost one channel is erased
  ...
  p: float
      Probability of image returned being flipped 
  channels_to_erase: list.
    Channels to erase, if specified else, all channels will used
  """

  def __init__(self,p=0.5,channels_to_erase=None):
    ...
    self.channels_to_erase=channels_to_erase
  ...
  def __call__(self, img):
    ...
    if self.channels_to_erase is None:
      channels_to_erase=list(range(channels))
    else:
      channels_to_erase=self.channels_to_erase

    if numpy.random.random() < self.p:
      # operate only if the random probability is 

      imgn=img.copy()

      selected_channel=numpy.random.choice(channels_to_erase)

      imgn[:,:,selected_channel]=imgn[:,:,selected_channel]*0
    else:
      imgn=img.copy()
  ...
  def __repr__(self):
    return self.__class__.__name__ + '(Flipping Probability={}, Channels to erase = {})'.format(self.p, self.channels_to_erase)



class RandomChannelSelect(object):

  """Select a channel from a multi channel image randomly. all other channels are zeros 
  ...
  def __init__(self,p=0.5,channels_to_select=None):
  ...
  def __call__(self, img):
    ...
    if self.channels_to_select is None:
      channels_to_select=list(range(channels))
    else:
    ...
    if numpy.random.random() < self.p:
      ...
      selected_channel=numpy.random.choice(channels_to_select)

      imgn[:,:,selected_channel]=imgc[:,:,selected_channel]
    else:
      imgn=img.copy()
  ...
  def __repr__(self):
    return self.__class__.__name__ + '(Flipping Probability={}, Channels to select = {})'.format(self.p, self.channels_to_select)
